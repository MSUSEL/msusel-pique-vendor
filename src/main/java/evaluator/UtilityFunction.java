...
public class UtilityFunction extends DefaultUtility {
    /**
    ...
    public double utilityFunction(double inValue, Double[] thresholds, boolean positive) {
        ...
        if (!positive) {
            if (inValue == 0) return 1.0;
            else if (inValue == thresholds[0] && inValue == thresholds[1]) return 0.0;
            else if (inValue <= thresholds[0]) return 1.0;
